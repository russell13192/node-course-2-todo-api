...
describe('GET /todos/:id', () => {
  it('should return todo doc', (done) => {
    ...
    it('should not return todo doc created by other user', (done) => {
        request(app).get(`/todos/${todos[1]._id.toHexString()}`).set('x-auth', users[0].tokens[0].token).expect(404).end(done);
    });
  ...
  it('should return 404 for non-object ids', (done) => {
    request(app).get('/todos/123abd').set('x-auth', users[0].tokens[0].token).expect(404).end(done);
  });
});
...
describe('PATCH /todos/:id', () => {
  ...
  it('should clear completedAt when todo is not completed', (done) => {
    var hexId = todos[1]._id.toHexString();
    var text = 'This should be the new text Bitch!!!!';
